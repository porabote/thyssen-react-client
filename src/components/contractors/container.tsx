...
import contractorsReducer from "./redux-store/reducer";
...
const Container = (props: IChildComponentProps): JSX.Element => {
  ...
  registrationReducer("contractors", contractorsReducer);
  registrationSaga("contractors", sagaWatcher);

  useEffect(() => {
    fetchData();
  }, []);

  const contractorsState = useAppSelector(state => state.contractors);
