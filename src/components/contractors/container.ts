...
const MailsContainer = (props: IChildComponentProps) => {
  ...
  const {dictsRequired, title, meta, filter} = useSelector(state => state.contractors);
  const {components, dicts} = useSelector(state => state.dicts);

  const isDictsLoaded = components.contractors ? true : false;

  useEffect(() => {
    dispatch(requestDicts(dictsRequired, 'contractors'));
    fetchData();
  }, []);
