...
import {DoneIcon, CloseIcon} from "@/app/ui/icons";
      ...
      <div>
        ...
        <Grid grid-template-columns="220px 1fr">
          ...
          {nomenclatures.map(item => {
            ...
            if (!item.purchase_request.psps.length) return;

            return (
              <div key={item.id}>
                <div>{item.purchase_request.id}</div>
                <div>{item.purchase_request.psps}</div>
              </div>
            );
          })}
        </Grid>
        ...
        {/*  <h4>Данные PSP</h4>*/}
        {/*</div>*/}
        <Grid grid-template-columns="120px 120px 1fr 120px 120px 120px">
